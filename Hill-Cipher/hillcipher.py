...
#    Kelas       : A
#    Program     : Hill Cipher
#    Tugas       : Tugas Pertemuan 3 Praktikum Kriptografi 
...
def char_to_number(x):
    """
    Mengubah karakter menjadi angka dengan mengurangi kode ASCII karakter 'A' dari kode ASCII karakter input.
    """
    x = ord(x) - 65
    return x
...
def number_to_char(x):
    """
    Mengubah angka menjadi karakter dengan menambahkan 65 ke angka input dan mengubahnya menjadi karakter.
    """
    x = chr(x + 65)
    return x
...
def mod_inverse(A, M):
    """
    Menghitung invers modulo dari A dengan modulus M.
    """
    for X in range(1, M):
        if (((A % M) * (X % M)) % M == 1):
            return X
...
def input_key(n):
    """
    Mengambil input matrix kunci dari pengguna dan mengubahnya menjadi array numpy dengan ukuran n x n.
    """
...
def input_text(string):
    """
    Mengambil input teks dari pengguna, menghapus spasi, dan mengubahnya menjadi huruf kapital.
    """
...
def hill(method, text, key, n):
    """
    Melakukan enkripsi atau dekripsi teks menggunakan matrix kunci yang diberikan.
    """
    ...
    # Memastikan panjang teks kelipatan dari n
    if len(text) % n != 0:
        last_char = text[-1]
        text = text + last_char * (n - len(text) % n)
...
def find_key(pt, ct, m):
    """
    Mencari matrix kunci dari plaintext dan ciphertext yang diberikan.
    """
